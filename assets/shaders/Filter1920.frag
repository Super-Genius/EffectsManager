uniform sampler2D inputImageTexture0;		// Y part of input video frame
uniform sampler2D inputImageTexture1;		// UV (CrCb) part of input video frame
...
const highp mat3 matyuv2rgb = mat3(   
									1.164,  1.164,  1.164,
                     				0.000, -0.392,  2.017,
               						1.596, -0.813,  0.000
                            	  ); 
...
highp vec3 YUVtoRGB()
{
    ...
    highp vec2 flipTexCoord = vec2(textureCoordinate.x, textureCoordinate.y);
    
    //return texture2D(inputImageTexture0, flipTexCoord).rgb;
 	
    YUV.r = texture2D(inputImageTexture0, flipTexCoord).r - 0.0625;
    YUV.bg = (texture2D(inputImageTexture1, flipTexCoord).ra) - 0.5;
                    
  	return matyuv2rgb * YUV;
 ...
 lowp float computeVignette()
{
 ...
 void main()
 {
     ...
     highp vec4 textureColor 	= vec4(YUVtoRGB(), 1.0);

     highp float m3 = mod( float( frameTime ), 1500.0 );
     if (1 == 1)
     {
         textureColor.rgb *= 1.0 - computeVignette();
     }
